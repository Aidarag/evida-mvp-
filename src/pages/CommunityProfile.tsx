...
export const CommunityProfile: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8 fade-in pb-24 text-left select-none bg-brand-bg">
      ...
      <div className="relative rounded-[2rem] overflow-hidden border border-white/10 bg-[#1A1214]/60 backdrop-blur-md shadow-sm">
        {/* Banner image */}
        ...
        <div className="p-6 sm:p-8 flex flex-col sm:flex-row items-center sm:items-end justify-between gap-6 -mt-10 sm:-mt-14 relative z-10 text-center sm:text-left">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 sm:gap-5">
            {/* Logo Emblem */}
            <div className="w-20 h-20 sm:w-24 sm:h-24 bg-[#1A1214] rounded-3xl border-4 border-white/10 shadow-2xl flex items-center justify-center select-none">
              {renderCommunityIcon(community.logo, "w-10 h-10 sm:w-12 sm:h-12 text-[#FE7F42]")}
            </div>
            ...
            <div className="text-brand-text sm:pb-1 space-y-1 text-left">
              ...
              <div className="flex items-center justify-start space-x-3 text-xs sm:text-sm text-brand-text-sec font-medium pt-1">
                <span className="bg-brand-bg px-2.5 py-0.5 rounded-full border border-white/10">
                  {community.category}
                </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        <div className="lg:col-span-1 bg-[#1A1214]/60 backdrop-blur-md p-5 rounded-3xl border border-white/10 shadow-sm space-y-1.5 select-none">
        ...
        <div className="lg:col-span-3 min-h-[300px]">
          ...
          {activeTab === 'about' && (
            <div className="space-y-8">
              {/* About description */}
              <div className="bg-[#1A1214]/60 backdrop-blur-md p-6 sm:p-8 rounded-[2rem] border border-white/10 shadow-sm space-y-4">
                <h3 className="text-2xl font-display text-brand-text border-b border-white/5 pb-2 uppercase tracking-tight">About Our Club</h3>
              ...
              <div className="space-y-4">
                <h3 className="text-2xl font-display text-brand-text border-b border-white/5 pb-2 uppercase tracking-tight">Upcoming Gatherings</h3>
                {communityEvents.length > 0 ? (
                  ...
                  <div className="bg-[#1A1214]/60 backdrop-blur-md p-6 rounded-3xl border border-white/10 text-center py-10 shadow-sm">
          ...
          {activeTab === 'discussions' && (
            <div className="space-y-6">
              ...
              <div className="bg-[#1A1214]/60 backdrop-blur-md p-5 rounded-3xl border border-white/10 shadow-sm">
              ...
              <div className="space-y-4">
                {community.discussions.length > 0 ? (
                  community.discussions.map((disc) => (
                    <div key={disc.id} className="bg-[#1A1214]/60 backdrop-blur-md p-5 rounded-3xl border border-white/10 shadow-sm flex items-start gap-4">
                      <div className="w-10 h-10 rounded-full overflow-hidden bg-brand-bg flex-shrink-0 border border-white/10">
          ...
          {/* MEMBERS SHEET TAB */}
          {activeTab === 'members' && (
            <div className="bg-[#1A1214]/60 backdrop-blur-md p-6 sm:p-8 rounded-[2rem] border border-white/10 shadow-sm space-y-6">
              <h3 className="text-2xl font-display text-brand-text border-b border-white/5 pb-2 uppercase tracking-tight">Active Board Members</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {community.membersList.map((memb, idx) => (
                  <div key={idx} className="flex items-center space-x-3.5 p-3 rounded-2xl border border-white/10 bg-brand-bg">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-brand-bg flex-shrink-0 border border-white/10">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-[#FE7F42] font-semibold truncate">{memb.role || 'Member'}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'projects' && (
            <div className="space-y-6">
              {/* Projects Grid */}
              <div className="space-y-4">
                <h3 className="text-2xl font-display text-brand-text border-b border-white/5 pb-2 uppercase tracking-tight">Featured Projects</h3>
                {community.projects.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {community.projects.map((proj) => (
                      <div key={proj.id} className="bg-[#1A1214]/60 backdrop-blur-md rounded-3xl overflow-hidden border border-white/10 shadow-sm flex flex-col h-full hover:shadow-md transition-all">
              ...
              <div className="space-y-4 pt-4">
                <h3 className="text-2xl font-display text-brand-text border-b border-white/5 pb-2 uppercase tracking-tight">Shared Resources</h3>
                  ...
                  <div className="space-y-3">
                    {community.resources.map((res) => (
                      <a 
                        key={res.id}
                        href={res.url}
                        className="bg-[#1A1214]/60 backdrop-blur-md px-5 py-3 rounded-2xl border border-white/10 shadow-sm flex items-center justify-between hover:border-[#FE7F42]/30 hover:shadow-md transition-all select-none group"
                      >
                        <div className="flex items-center space-x-3">
                          <div className="w-9 h-9 rounded-xl bg-[#FE7F42]/10 flex items-center justify-center text-[#FE7F42] border border-[#FE7F42]/15">
                            <FileText className="w-5 h-5" />
                          </div>
                          <div>
                            <h4 className="text-xs font-bold text-brand-text group-hover:text-[#FE7F42] transition-colors">{res.title}</h4>
                            <span className="text-[9px] text-[#FE7F42] font-medium uppercase tracking-wider">{res.category}</span>
                          </div>
                        </div>
                        <span className="text-[11px] text-[#FE7F42] font-bold">Download ↗</span>
                      </a>
                    ))}
                  </div>
                ) : (
          ...
          {activeTab === 'gallery' && (
            <div className="bg-[#1A1214]/60 backdrop-blur-md p-6 sm:p-8 rounded-[2rem] border border-white/10 shadow-sm space-y-6">
              <h3 className="text-2xl font-display text-brand-text border-b border-white/5 pb-2 uppercase tracking-tight">Club Gallery</h3>
              {community.gallery.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 select-none">
                  {community.gallery.map((imgUrl, idx) => (
                    <div key={idx} className="aspect-square rounded-2xl overflow-hidden border border-white/10 bg-brand-bg relative group">
