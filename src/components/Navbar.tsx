...
import { Compass, Users, User, Home, Briefcase, PlusCircle } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  const desktopNavItems: { label: string; page: PageName; icon: React.ReactNode }[] = [
    ...
    { label: 'Communities', page: 'communities', icon: <Users className="w-4 h-4" /> },
    { label: 'Opportunities', page: 'opportunities', icon: <Briefcase className="w-4 h-4" /> },
    { label: 'Join Evida', page: 'profile', icon: <User className="w-4 h-4" /> },
  ];
  ...
  const mobileBottomItems: { label: string; page: PageName; icon: React.ReactNode }[] = [
    ...
    { label: 'Events', page: 'explore', icon: <Compass className="w-5.5 h-5.5" /> },
    { label: 'Communities', page: 'communities', icon: <Users className="w-5.5 h-5.5" /> },
    { label: 'Opportunities', page: 'opportunities', icon: <Briefcase className="w-5.5 h-5.5" /> },
    { label: 'Join Evida', page: 'profile', icon: <User className="w-5.5 h-5.5" /> },
  ];
  ...
  return (
    <>
      ...
      <nav className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-100 text-brand-text transition-all duration-300">
              ...
              {desktopNavItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center space-x-1.5 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 font-display cursor-pointer ${
                      isActive
                        ? 'bg-brand-purple/10 text-brand-purple font-bold shadow-sm'
                        : 'text-brand-text-sec hover:text-brand-text hover:bg-brand-purple/5'
                    }`}
                  >
                    {item.icon}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-xl border-t border-slate-100 py-2.5 px-6 flex justify-between items-center shadow-xl rounded-t-3xl pb-safe">
        {mobileBottomItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] font-display font-bold mt-1 tracking-tight ${isActive ? 'text-brand-purple' : 'text-brand-text/45'}`}>
                {item.label === 'Join Evida' ? 'Profile' : item.label}
              </span>
